...
race_plurality <- combined %>% 
  ...
  mutate(plurality = as_factor(variable)) %>% 
  select(-variable)
...
recipe_1 <- recipe(distance ~  white + black + asian + hispanic,
                   data = model_train)
...
recipe_4 <- recipe(distance ~ white + black + asian + hispanic + plurality,
                   data = model_data) %>% 
  step_dummy(plurality)

recipe_5 <- recipe(distance ~ white + black + asian + hispanic + plurality,
                   data = model_data) %>% 
  step_dummy(plurality) %>% 
  step_poly(white:hispanic, degree = 3)
...
fit <- stan_glm(data = model_data, formula = distance ~ white + black + asian + 
...
# example, row 3 represents majority Asian tracts, and the population proportions
# for row three are the average proportions for majority Asian tracts.
...
tbl <- fit %>% 
  ...
  tab_source_note("*Tracts up to 3218 meters (2 miles) from nearest station")
